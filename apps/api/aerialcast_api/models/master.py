"""Core master data models such as users, drones, and checklists."""

from datetime import datetime
import uuid
from typing import List, Optional, TYPE_CHECKING

from sqlalchemy import Enum, DateTime, Float, ForeignKey, SmallInteger, String, Integer, Text
from sqlalchemy.dialects.postgresql import UUID
from sqlalchemy.orm import Mapped, mapped_column, relationship

from ..extensions import db
from .enums import ChecklistType, DroneStatus, GeofenceType, UserRole
...
class Drone(db.Model):
    ...
    flight_sessions: Mapped[List["FlightSession"]] = relationship(back_populates="drone")
    maintenance_logs: Mapped[List["MaintenanceLog"]] = relationship(
        back_populates="drone"
    )
    missions: Mapped[List["Mission"]] = relationship(back_populates="drone")
    specs: Mapped[Optional["DroneSpecs"]] = relationship(
        back_populates="drone", uselist=False, cascade="all, delete-orphan"
    )
...
class DroneSpecs(db.Model):
    ...
    drone_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("drones.drone_id"), unique=True, nullable=False
    )
    ...
    drone: Mapped[Drone] = relationship(back_populates="specs")
