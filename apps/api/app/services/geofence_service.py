...
def _parse_geofence_type(value):
    ...
    if isinstance(value, str):
        ...
        try:
            ...
            for gt in GeofenceType:
                if gt.value.upper() == candidate:
                    return gt
...
class GeofenceService:
    ...
    def create_geofence(data):
        ...
        try:
            ...
            return {"error": str(e), "allowed_types": [gt.value for gt in GeofenceType]}, 400
        ...
        for idx, p in enumerate(points_data):
            ...
            for key in ("latitude", "longitude", "order"):
                if key not in p:
                    return {"error": f"Point at index {idx} must include field '{key}'"}, 400
            lat = p["latitude"]
            lon = p["longitude"]
            order = p["order"]
            if not isinstance(lat, (int, float)) or not isinstance(lon, (int, float)):
                return {"error": f"Point at index {idx} latitude/longitude must be numeric"}, 400
        ...
        try:
            ...
            db.session.rollback()
            return {"error": "Integrity error: possible constraint violation", "detail": str(ie)}, 400
    ...
    @staticmethod
    def update_geofence(geofence_id, data):
        geofence = Geofence.query.get_or_404(geofence_id)
        ...
        if "points" in data:
            ...
            orders = [p["order"] for p in points_data if "order" in p]
            if len(orders) != len(set(orders)):
                return {"error": "Duplicate point order values are not allowed"}, 400
            for p in points_data:
                new_point = GeofencePoint()
                new_point.latitude = p["latitude"]
                new_point.longitude = p["longitude"]
                new_point.order = p["order"]
