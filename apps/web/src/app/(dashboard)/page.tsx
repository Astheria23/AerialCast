...
import { Activity, ClipboardCheck, ListChecks, Loader2, Map, MapPin, Plane, Zap } from "lucide-react"
      ...
      <section className="rounded-2xl border border-border bg-card/80 p-8 shadow-sm">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <h1 className="mt-2 text-4xl font-bold tracking-tight">Welcome back, {userFirstName}</h1>
            <p className="mt-3 max-w-2xl text-base text-muted-foreground">
              Track mission queues, fleet readiness, and safety coverage from this central mission control.
            </p>
          </div>
      ...
      <section className="grid gap-4 md:grid-cols-2">
        <Card className="border border-border/80">
          <CardHeader>
            ...
            <CardDescription>Open requests currently awaiting approval.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            {missionsLoading ? (
              <div className="flex items-center gap-3 text-sm text-muted-foreground">
                <Loader2 className="h-4 w-4 animate-spin" /> Fetching missions...
              </div>
            ) : latestPendingMissions.length > 0 ? (
              <ul className="space-y-3">
                {latestPendingMissions.map((mission) => {
                  const createdAt = mission.created_at ? new Date(mission.created_at).toLocaleString() : "—"
                  ...
                  return (
                    ...
                    >
                      <div className="space-y-1">
                        <p className="text-sm font-semibold text-foreground">{mission.mission_name}</p>
                        <p className="text-xs text-muted-foreground">Pilot {mission.pilot_name ?? "Unassigned"}</p>
                        <p className="text-xs text-muted-foreground">Drone {droneLabel}</p>
                        <p className="text-xs text-muted-foreground">Submitted {createdAt}</p>
                      </div>
              ...
              <div className="rounded-lg border border-dashed border-border/70 bg-card/50 p-4 text-sm text-muted-foreground">
                No pending missions at the moment. Use the button below to plan the next sortie.
              </div>
            )}
          </CardContent>
        ...
        <Card className="border border-border/80">
          <CardHeader>
            ...
            <CardDescription>Aircraft available for new assignments.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            {dronesLoading ? (
              <div className="flex items-center gap-3 text-sm text-muted-foreground">
                <Loader2 className="h-4 w-4 animate-spin" /> Checking drone status...
              </div>
              ...
              <div className="rounded-lg border border-dashed border-border/70 bg-card/50 p-4 text-sm text-muted-foreground">
                No ready drones right now. Check maintenance status or active flight sessions.
              </div>
            )}
            {!dronesLoading && busyDrones.length > 0 && (
              <p className="text-xs text-muted-foreground">
                {busyDrones.length} drone(s) currently busy or undergoing maintenance.
              </p>
            )}
          </CardContent>
          <CardFooter className="justify-end">
          ...
          return (
            <Card key={item.title} className="border border-border/60">
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                ...
                <span className={`flex h-10 w-10 items-center justify-center rounded-full ${item.accent}`}>
